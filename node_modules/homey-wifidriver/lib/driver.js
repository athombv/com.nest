...
class HomeyWifiDriver extends Homey.Driver {
	...
	customOAuth2OnPair(socket) {
		...
		if (this.oauth2Client.registeredAccounts().length === 0 || (this.oauth2Client.registeredAccounts().length === 1 && typeof this.oauth2Client.getAccount().accessToken === 'undefined') ||this.oauth2ClientConfig.allowMultipleAccounts) {
			Util.generateOAuth2Callback(oauth2Account, socket);
		} else {
