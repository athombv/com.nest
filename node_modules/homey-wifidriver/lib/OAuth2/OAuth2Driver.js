...
class OAuth2Driver extends WifiDriver {
	...
	customOAuth2OnPair(socket) {
		...
		// Listen for the authorization show view event (prevents race)
		socket.on('showView', ( viewId, callback ) => {
			...
			if (viewId === 'authorization') {
				...
				if (this.oauth2Client.registeredAccounts().length === 0 ||
					...
					socket.emit('authorized'); // TODO: this event is emitted but seems to never arrive in oauth2_login template
				}
			}
		});
